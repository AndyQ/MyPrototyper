...
#define kGradientTitleTopColor [UIColor colorWithWhite:0.85f alpha:kBoxAlpha]
...
{
    UIColor *gradientBottomColour;
    UIColor *gradientTopColour;
}
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text {
    ...
    CGSize size = CGSizeMake([[UIScreen mainScreen] bounds].size.width - kHorizontalMargin*4.f, 1000.f);
    NSMutableParagraphStyle * paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;
    paragraphStyle.alignment = NSTextAlignmentLeft;
    NSDictionary * attributes = @{NSFontAttributeName : font,
                                  NSParagraphStyleAttributeName : paragraphStyle};
    CGSize textSize = [text boundingRectWithSize:size options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray {
    ...
    for(NSString *string in stringArray) {
        ...
        textSize.height = MAX( 44, textSize.height);
...
- (void)showImage:(UIImage *)image withMessage:(NSString *)msg {
    ...
    imageView.alpha = 0.f;
    imageView.frame = CGRectMake(CGRectGetMidX(_contentView.bounds) - image.size.width*0.5f, CGRectGetMidY(_contentView.bounds) - image.size.height*0.5f + 20.f, image.size.width, image.size.height);
...
- (void)showError {
    ...
    imageView.alpha = 0.f;
    imageView.frame = CGRectMake(CGRectGetMidX(_contentView.bounds) - 20.f, CGRectGetMidY(_contentView.bounds) - 20.f + 20.f, 40.f, 40.f);
    imageView.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    for(int i = 0; i < _subviewsArray.count && !found; i++) {
        ...
        if(CGRectContainsPoint(view.frame, point)) {
            ...
            if(_delegate && [_delegate respondsToSelector:@selector(popoverView:didSelectItemAtIndex: itemText:)]) {
                [_delegate popoverView:self didSelectItemAtIndex:i itemText:text];
            }
